...
async def get_stats_overview(request: Request):
    ...
    today_counts = await fetch_row(
        ...
        """
        SELECT
            (SELECT COUNT(*)::int FROM meetings WHERE meeting_date = CURRENT_DATE) AS meetings_today,
            (SELECT COUNT(*)::int FROM races r JOIN meetings m ON m.id = r.meeting_id WHERE m.meeting_date = CURRENT_DATE) AS races_today,
            (SELECT COUNT(*)::int FROM race_entries re JOIN races r ON r.id = re.race_id JOIN meetings m ON m.id = r.meeting_id WHERE m.meeting_date = CURRENT_DATE) AS runners_today
        """,
    ) or {"meetings_today": 0, "races_today": 0, "runners_today": 0}
    freshness = await fetch_row(
        request,
        """
        SELECT
    ...
    ingestion = await fetch_row(
        ...
        """
        SELECT
            (SELECT COUNT(*)::int FROM odds_snapshots WHERE captured_at >= NOW() - INTERVAL '24 hours') AS odds_snapshots_24h,
            (SELECT COUNT(*)::int FROM weather_readings WHERE recorded_at >= NOW() - INTERVAL '24 hours') AS weather_readings_24h,
            (SELECT COUNT(*)::int FROM media_segments WHERE segment_started_at >= NOW() - INTERVAL '24 hours') AS media_segments_24h
        """,
    ) or {"odds_snapshots_24h": 0, "weather_readings_24h": 0, "media_segments_24h": 0}
    total_db_size = await fetch_value(
        request,
